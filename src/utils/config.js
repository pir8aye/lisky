...
const attemptCallWithWarning = (fn, path) => {
	try {
		...
		const warning = `WARNING: Could not write to \`${
			path
...
const checkReadAccess = path => {
	...
	const error = `Could not read config file. Please check permissions for ${
		path
...
const checkLockfile = path => {
	...
	const error = `Config lockfile at ${
		lockfilePath
	} found. Are you running Lisky in another process?`;
	return attemptCallWithError(fn, 3, error);
};

const attemptToReadJsonFile = path => {
	const fn = readJsonSync.bind(null, path);
	const error = `Config file is not valid JSON. Please check ${
		path
	} or delete the file so we can create a new one from defaults.`;
