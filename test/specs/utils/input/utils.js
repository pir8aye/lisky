			...
			Given(
				...
				() => {
					describe('#getPassphraseFromEnvVariable', () => {
						describe(`Given the passphrase is stored in environmental variable "${
							ENV_VARIABLE
						}"`, () => {
						...
						describe(`Given environmental variable "${
							ENV_VARIABLE
						}" is not set`, () => {
						...
						Given(
							...
							() => {
								When(
									...
									() => {
										Then(
											'it should reject with message "Unknown data source type. Must be one of `file`, or `stdin`."',
											then.itShouldRejectWithMessage,
										);
									},
								);
